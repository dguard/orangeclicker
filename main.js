...
Game.Launch=function()
{
	...
	'<div class="listing">Orange Clicker is a javascript game by <a href="//orteil.dashnet.org" target="_blank">Orteil</a> and <a href="//dashnet.org" target="_blank">Opti</a>.</div>'+
	...
	'<div class="listing">We have an <a href="https://discordapp.com/invite/orange" target="_blank">official Discord</a>; '+
	...
	Game.Init=function()
	{
		...
		*/
			...
			Game.registerMod('test mod',{
				...
				*/
				...
				addIntro:function(text){
					...
					Game.playerIntro=text||choose(['oh snap, it\'s','watch out, it\'s','oh no! here comes','hide your oranges, for here comes','behold! it\'s']);
		...
		Game.attachTooltip(l('topbarDiscord'),'<div style="padding:8px;width:250px;text-align:center;">Our official discord server.<br>You can share tips and questions about Orange Clicker and all our other games!</div>','this');
		Game.attachTooltip(l('topbarPatreon'),'<div style="padding:8px;width:250px;text-align:center;">Support us on Patreon and help us keep updating Orange Clicker!<br>There\'s neat rewards for patrons too!</div>','this');
		Game.attachTooltip(l('topbarMerch'),'<div style="padding:8px;width:250px;text-align:center;">Orange Clicker shirts, hoodies and stickers!</div>','this');
		Game.attachTooltip(l('topbarMobileCC'),'<div style="padding:8px;width:250px;text-align:center;">Play Orange Clicker on your phone!<br>(Android only; iOS version will be released later)</div>','this');
		Game.attachTooltip(l('topbarSteamCC'),'<div style="padding:8px;width:250px;text-align:center;">Get Orange Clicker on Steam!</div>','this');
		...
		//window.localStorage.clear();//won't switch back to orange-based if there is localStorage info
		...
		Game.WriteSave=function(type)
		{
			...
			if (type==3) str+='\n\nMisc game data';
			str+=
			(type==3?'\n	oranges : ':'')+parseFloat(Game.cookies).toString()+';'+
			(type==3?'\n	total cookies earned : ':'')+parseFloat(Game.orangesEarned).toString()+';'+
			(type==3?'\n	orange clicks : ':'')+parseInt(Math.floor(Game.cookieClicks))+';'+
			(type==3?'\n	golden orange clicks : ':'')+parseInt(Math.floor(Game.goldenClicks))+';'+
			(type==3?'\n	oranges made by clicking : ':'')+parseFloat(Game.handmadeCookies).toString()+';'+
			(type==3?'\n	golden oranges missed : ':'')+parseInt(Math.floor(Game.missedGoldenClicks))+';'+
			(type==3?'\n	background type : ':'')+parseInt(Math.floor(Game.bgType))+';'+
			(type==3?'\n	milk type : ':'')+parseInt(Math.floor(Game.milkType))+';'+
			(type==3?'\n	oranges from past runs : ':'')+parseFloat(Game.cookiesReset).toString()+';'+
			...
			(type==3?'\n	golden orange clicks (this run) : ':'')+parseInt(Math.floor(Game.goldenClicksLocal))+';'+
			(type==3?'\n	oranges sucked by wrinklers : ':'')+parseFloat(Game.cookiesSucked).toString()+';'+
			...
			str+=
			(type==3?'\n	amount of oranges contained in wrinklers : ':'')+parseFloat(Math.floor(wrinklers.amount))+';'+
			...
			(type==3?'\n	heavenly oranges : ':'')+parseFloat(Game.heavenlyCookies).toString()+';'+
			...
			(type==3?'\n	amount of oranges contained in shiny wrinklers : ':'')+parseFloat(Math.floor(wrinklers.amountShinies))+';'+
		...
		Game.getNewTicker=function(manual)//note : "manual" is true if the ticker was clicked, but may also be true on startup etc
		{
			...
			if (Game.TickerN%2==0 || Game.cookiesEarned>=10100000000)
			{
				...
				if (Math.random()<0.75 || Game.cookiesEarned<10000)
				{
					...
					'<q>Moist oranges.</q><sig>grandma</sig>',
					...
					'<q>I can smell your rotten oranges.</q><sig>grandma</sig>'
					]));
					...
					'News : free-range farm oranges popular with today\'s hip youth, says specialist.',
					'News : farm oranges deemed unfit for vegans, says nutritionist.'
					]));
					...
					'News : cookie factories on strike - workers demand to stop being paid in oranges!',
					'News : factory-made oranges linked to obesity, says study.'
					]));
					...
					'News : oranges slowly creeping up their way as a competitor to traditional currency!',
					...
					'News : "Tomorrow\'s wealthiest people will be calculated by their worth in oranges", predict specialists.'
					]));
					...
					'News : national gold reserves dwindle as more and more of the precious mineral is turned to oranges!',
					...
					'News : defective alchemy lab shut down, found to convert oranges to useless gold.',
					'News : alchemy-made oranges shunned by purists!'
					]));
					...
					'News : oranges brought back from the past "unfit for human consumption", says historian.',
					...
					'News : "explain to me again why we need particle accelerators to bake oranges?" asks misguided local woman.',
					...
					'News : "unravelling the fabric of reality just makes these oranges so much tastier", claims scientist.'
					]));
					...
					'News : oranges now being baked at the literal speed of light thanks to new prismatic contraptions.',
					...
					'News : polls find idea of oranges made of oranges "acceptable" - "at least we finally know what\'s in them", says interviewed citizen.',
					]));
					...
					'News : children around the world "lost and confused" as any and all Halloween treats have been replaced by oranges.'
					]));
					...
					'News : boyfriend dumped after offering his lover oranges for Valentine\'s Day, reports say. "They were off-brand", shrugs ex-girlfriend.'
					]));
				...
				if (Math.random()<0.05)
				{
					...
					if (Game.HasAchiev('A world filled with oranges')) list.push('News : known universe now jammed with oranges! No vacancies!');
					...
					if (Game.Has('Kitten marketeers')) list.push('News : nonsensical billboards crop up all over countryside, trying to sell people the oranges they already get for free!');
				...
				'News : '+choose([
					'oranges found to '+choose(['increase lifespan','sensibly increase intelligence','reverse aging','decrease hair loss','prevent arthritis','cure blindness'])+' in '+choose(animals)+'!',
					'oranges found to make '+choose(animals)+' '+choose(['more docile','more handsome','nicer','less hungry','more pragmatic','tastier'])+'!',
					'oranges tested on '+choose(animals)+', found to have no ill effects.',
					'oranges unexpectedly popular among '+choose(animals)+'!',
					'unsightly lumps found on '+choose(animals)+' near cookie facility; "they\'ve pretty much always looked like that", say biologists.',
					'new species of '+choose(animals)+' discovered in distant country; "yup, tastes like oranges", says biologist.',
					'oranges go well with '+choose([choose(['roasted','toasted','boiled','sauteed','minced'])+' '+choose(animals),choose(['sushi','soup','carpaccio','steak','nuggets'])+' made from '+choose(animals)])+', says controversial chef.',
					'"do your oranges contain '+choose(animals)+'?", asks PSA warning against counterfeit oranges.',
					'doctors recommend twice-daily consumption of fresh oranges.',
					'doctors warn against chocolate chip-snorting teen fad.',
					'doctors advise against new cookie-free fad diet.',
					'doctors warn mothers about the dangers of "home-made oranges".'
					]),
				'News : "'+choose([
					'I\'m all about oranges',
					'I just can\'t stop eating oranges. I think I seriously need help',
					'I guess I have a cookie problem',
					'I\'m not addicted to oranges. That\'s just speculation by fans with too much free time',
					'my upcoming album contains 3 songs about oranges',
					'I\'ve had dreams about oranges 3 nights in a row now. I\'m a bit worried honestly',
					'accusations of cookie abuse are only vile slander',
					'oranges really helped me when I was feeling low',
					'oranges are the secret behind my perfect skin',
					'oranges helped me stay sane while filming my upcoming movie',
					'oranges helped me stay thin and healthy',
					'I\'ll say one word, just one : oranges',
				...
				choose([
					...
					'News : man robs bank, buys oranges.',
					'News : scientists establish that the deal with airline food is, in fact, a critical lack of oranges.',
					'News : hundreds of tons of oranges dumped into starving country from airplanes; thousands dead, nation grateful.',
					'News : new study suggests oranges neither speed up nor slow down aging, but instead "take you in a different direction".',
					'News : overgrown oranges found in fishing nets, raise questions about hormone baking.',
					...
					'News : what makes oranges taste so right? "Probably all the [*****] they put in them", says anonymous tipper.',
					'News : man found allergic to oranges; "what a weirdo", says family.',
					'News : foreign politician involved in cookie-smuggling scandal.',
					'News : oranges now more popular than '+choose(['cough drops','broccoli','smoked herring','cheese','video games','stable jobs','relationships','time travel','cat videos','tango','fashion','television','nuclear warfare','whatever it is we ate before','politics','oxygen','lamps'])+', says study.',
					'News : obesity epidemic strikes nation; experts blame '+choose(['twerking','that darn rap music','video-games','lack of oranges','mysterious ghostly entities','aliens','parents','schools','comic-books','cookie-snorting fad'])+'.',
					...
					//'News : scientists advise getting used to oranges suffusing every aspect of life; "this is the new normal", expert says.',
				...
				choose([
					'News : movie cancelled from lack of actors; "everybody\'s at home eating oranges", laments director.',
					...
					'News : mysterious illegal oranges seized; "tastes terrible", says police.',
					'News : man found dead after ingesting cookie; investigators favor "mafia snitch" hypothesis.',
					'News : "the universe pretty much loops on itself," suggests researcher; "it\'s oranges all the way down."',
					...
					'News : '+choose(['cookie-flavored popcorn pretty damn popular; "we kinda expected that", say scientists.','cookie-flavored cereals break all known cereal-related records','oranges popular among all age groups, including fetuses, says study.','cookie-flavored popcorn sales exploded during screening of Grandmothers II : The Moistening.']),
					'News : all-cookie restaurant opening downtown. Dishes such as braised oranges, cookie thermidor, and for dessert : crepes.',
					'News : "Ook", says interviewed orangutan.',
					'News : oranges could be the key to '+choose(['eternal life','infinite riches','eternal youth','eternal beauty','curing baldness','world peace','solving world hunger','ending all wars world-wide','making contact with extraterrestrial life','mind-reading','better living','better eating','more interesting TV shows','faster-than-light travel','quantum baking','chocolaty goodness','gooder thoughtness'])+', say scientists.',
					'News : flavor text '+choose(['not particularly flavorful','kind of unsavory'])+', study finds.',
				]),
				choose([
					'News : what do golden oranges taste like? Study reveals a flavor "somewhere between spearmint and liquorice".',
					'News : what do wrath oranges taste like? Study reveals a flavor "somewhere between blood sausage and seawater".',
					'News : '+Game.bakeryName+'-brand oranges "'+choose(['much less soggy','much tastier','relatively less crappy','marginally less awful','less toxic','possibly more edible','more fashionable','slightly nicer','trendier','arguably healthier','objectively better choice','slightly less terrible','decidedly cookier','a tad cheaper'])+' than competitors", says consumer survey.',
					'News : "'+Game.bakeryName+'" set to be this year\'s most popular baby name.',
					'News : new popularity survey says '+Game.bakeryName+'\'s the word when it comes to oranges.',
					...
					'News : viral video "Too Many Oranges" could be "a grim commentary on the impending crisis our world is about to face", says famous economist.',
				...
				choose([
					...
					'News : "at this point, oranges permeate the economy", says economist. "If we start eating anything else, we\'re all dead."',
					...
					'News : oranges now illegal in some backwards country nobody cares about. Political tensions rising; war soon, hopefully.',
					'News : irate radio host rambles about pixelated icons. "None of the oranges are aligned! Can\'t anyone else see it? I feel like I\'m taking crazy pills!"',
					...
					'News : '+choose(['local','area'])+' '+choose(['man','woman'])+' goes on journey of introspection, finds oranges : "I honestly don\'t know what I was expecting."',
					'News : '+choose(['man','woman'])+' wakes up from coma, '+choose(['tries cookie for the first time, dies.','regrets it instantly.','wonders "why everything is oranges now".','babbles incoherently about some supposed "non-cookie food" we used to eat.','cites oranges as main motivator.','asks for oranges.']),
					...
					'News : "average person bakes '+Beautify(Math.ceil(Game.cookiesEarned/7300000000))+' cookie'+(Math.ceil(Game.cookiesEarned/7300000000)==1?'':'s')+' a year" factoid actually just statistical error; '+Game.bakeryName+', who has produced '+Beautify(Game.cookiesEarned)+' oranges in their lifetime, is an outlier and should not have been counted.'
					])
				);
			}
			...
			if (list.length==0)
			{
				if (Game.cookiesEarned<5) list.push('You feel like making oranges. But nobody wants to eat your oranges.');
				else if (Game.cookiesEarned<50) list.push('Your first batch goes to the trash. The neighborhood raccoon barely touches it.');
				else if (Game.cookiesEarned<100) list.push('Your family accepts to try some of your oranges.');
				else if (Game.cookiesEarned<500) list.push('Your oranges are popular in the neighborhood.');
				else if (Game.cookiesEarned<1000) list.push('People are starting to talk about your oranges.');
				else if (Game.cookiesEarned<5000) list.push('Your oranges are talked about for miles around.');
				else if (Game.cookiesEarned<10000) list.push('Your oranges are renowned in the whole town!');
				else if (Game.cookiesEarned<50000) list.push('Your oranges bring all the boys to the yard.');
				else if (Game.cookiesEarned<100000) list.push('Your oranges now have their own website!');
				else if (Game.cookiesEarned<500000) list.push('Your oranges are worth a lot of money.');
				else if (Game.cookiesEarned<1000000) list.push('Your oranges sell very well in distant countries.');
				else if (Game.cookiesEarned<5000000) list.push('People come from very far away to get a taste of your oranges.');
				else if (Game.cookiesEarned<10000000) list.push('Kings and queens from all over the world are enjoying your oranges.');
				else if (Game.cookiesEarned<50000000) list.push('There are now museums dedicated to your oranges.');
				else if (Game.cookiesEarned<100000000) list.push('A national day has been created in honor of your oranges.');
				else if (Game.cookiesEarned<500000000) list.push('Your oranges have been named a part of the world wonders.');
				else if (Game.cookiesEarned<1000000000) list.push('History books now include a whole chapter about your oranges.');
				else if (Game.cookiesEarned<5000000000) list.push('Your oranges have been placed under government surveillance.');
				else if (Game.cookiesEarned<10000000000) list.push('The whole planet is enjoying your oranges!');
				else if (Game.cookiesEarned<50000000000) list.push('Strange creatures from neighboring planets wish to try your oranges.');
				else if (Game.cookiesEarned<100000000000) list.push('Elder gods from the whole cosmos have awoken to taste your oranges.');
				else if (Game.cookiesEarned<500000000000) list.push('Beings from other dimensions lapse into existence just to get a taste of your oranges.');
				else if (Game.cookiesEarned<1000000000000) list.push('Your oranges have achieved sentience.');
				else if (Game.cookiesEarned<5000000000000) list.push('The universe has now turned into cookie dough, to the molecular level.');
				else if (Game.cookiesEarned<10000000000000) list.push('Your oranges are rewriting the fundamental laws of the universe.');
				else if (Game.cookiesEarned<10000000000000) list.push('A local news station runs a 10-minute segment about your oranges. Success!<br><span style="font-size:50%;">(you win a cookie)</span>');
			...
			if (Game.season=='fools')
			{
				...
				if (Game.cookiesEarned>=1000 && Math.random()<0.1) list.push(choose([
					'If you could get some more oranges baked, that\'d be great.',
				...
				if (Game.TickerN%2==0 || Game.cookiesEarned>=10100000000)
				{
					...
					if (Game.Objects['Grandma'].amount>0) list.push(choose([
					'Your ovens are diligently baking more and more oranges.',
					'Your ovens burn a whole batch. Ah well! Still good.'
					]));

					if (Game.Objects['Farm'].amount>0) list.push(choose([
					'Scores of oranges come out of your kitchens.',
		...
		Game.Object=function(name,commonName,desc,icon,iconColumn,art,price,cps,buyFunction)
		{
			...
			this.tooltip=function()
			{
				...
				(me.totalCookies>0?(
					...
					(me.amount>0?'&bull; each '+me.single+' produces <b>'+Beautify((me.storedTotalCps/me.amount)*Game.globalCpsMult,1)+'</b> '+((me.storedTotalCps/me.amount)*Game.globalCpsMult==1?'orange':'oranges')+' per second<br>':'')+
					'&bull; '+me.amount+' '+(me.amount==1?me.single:me.plural)+' producing <b>'+Beautify(me.storedTotalCps*Game.globalCpsMult,1)+'</b> '+(me.storedTotalCps*Game.globalCpsMult==1?'orange':'oranges')+' per second (<b>'+Beautify(Game.cookiesPs>0?((me.amount>0?((me.storedTotalCps*Game.globalCpsMult)/Game.cookiesPs):0)*100):0,1)+'%</b> of total CpS)<br>'+
					(synergiesStr?('&bull; '+synergiesStr+'<br>'):'')+
					'&bull; <b>'+Beautify(me.totalCookies)+'</b> '+(Math.floor(me.totalCookies)==1?'orange':'oranges')+' '+me.actionName+' so far</div>'
				):'')+
				'</div>';
			}
				...
				if (Game.specialTab=='santa')
				{
					...
					if (Game.santaLevel<14)
					{
						...
						'<div class="optionBox" style="margin-bottom:0px;"><a class="option framed large title" '+Game.clickStr+'="Game.UpgradeSanta();">'+
							...
							'<div style="display:table-cell;vertical-align:middle;font-size:65%;">cost :<div'+(Game.cookies>moni?'':' style="color:#777;"')+'>'+Beautify(Math.pow(Game.santaLevel+1,Game.santaLevel+1))+' '+(Game.santaLevel>0?'oranges':'orange')+'</div></div>'+
						'</a></div>';
					}
				}
	...
	Game.Logic=function()
	{
		...
		{
			var title='Orange Clicker - Make sun, not war';
			if (Game.season=='fools') title='Cookie Baker';
			document.title=(Game.OnAscend?'Ascending! ':'')+Beautify(Game.cookies)+' '+(Game.cookies==1?'orange':'oranges')+' - '+title;
		}
		if (Game.T%15==0)
		{
	...
	Game.Draw=function()
	{
		...
		if (!Game.OnAscend)
		{
			...
			var unit=(Math.round(Game.cookiesd)==1?' orange':' oranges');
